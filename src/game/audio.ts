...
const TUNES: Record<Exclude<Tune, null>, TuneDef> = {
  ...
  floor1: {
    ...
    bass: ['A', 'F', 'G', 'E'].map((r) => bar(r, 'R2 . R3 . R2 . R3 . R2 . R3 . R2 . R3 .')).join(' '),
  ...
  floor2: {
    ...
    bass: ['D', 'G', 'D', 'C'].map((r) => bar(r, 'R2 . . R3 . . R2 . R2 . R3 . R2 . R3 .')).join(' '),
...
class ChipAudio {
  ...
  private seq: { def: TuneDef; lead: string[]; bass: string[]; drums: string[]; step: number; next: number } | null = null
  ...
  setMuted(m: boolean) {
    ...
    if (this.ctx && this.master) this.master.gain.setTargetAtTime(m ? 0 : 1, this.ctx.currentTime, 0.02)
  }

  /** quieter music (pause menu) */
  duck(on: boolean) {
    if (this.ctx && this.musicBus) this.musicBus.gain.setTargetAtTime(on ? MUSIC_VOL * 0.3 : MUSIC_VOL, this.ctx.currentTime, 0.05)
  }
  ...
  private pump() {
    ...
    while (s.next < ctx.currentTime + 0.12) {
      this.voice(s.lead, s.step, s.next, stepDur, 'square', s.def.leadVol)
      ...
      else if (d === 's') this.noise(s.next, 0.09, 0.12, 'bandpass', 1800, this.musicBus)
      else if (d === 'h') this.noise(s.next, 0.03, 0.05, 'highpass', 7000, this.musicBus)
      s.step++
      s.next += stepDur
    }
  }

  private voice(track: string[], step: number, t: number, stepDur: number, wave: Wave, vol: number) {
    const i = step % track.length
    const tok = track[i]
    if (tok === '.' || tok === '-' || !tok) return
    let len = 1
    while (track[(i + len) % track.length] === '-' && len < track.length) len++
    this.tone(noteFreq(tok), t, len * stepDur * 0.92, wave, vol, this.musicBus)
  }
  ...
  private tone(freq: number, t: number, dur: number, wave: Wave, vol: number, bus: GainNode | null, slideTo?: number) {
    const ctx = this.ctx
  ...
  private noise(t: number, dur: number, vol: number, filter: BiquadFilterType, freq: number, bus: GainNode | null, freqTo?: number) {
    const ctx = this.ctx
  ...
  private arp(notes: string[], t: number, each: number, wave: Wave, vol: number, lastHold = 1) {
    notes.forEach((n, i) => {
  ...
  play(name: Sfx) {
    ...
    switch (name) {
      ...
      case 'crash':
        ...
        break
      case 'jackpot':
        for (let i = 0; i < 6; i++) this.tone(i % 2 ? 1319 : 1568, t + i * 0.06, 0.06, 'square', 0.09, bus)
        break
      case 'round':
      ...
      case 'bossIntro':
        ...
        break
      case 'win':
        this.arp(['C5', 'E5', 'G5', 'C6', 'G5', 'C6'], t, 0.11, 'square', 0.15, 4)
        this.arp(['C3', 'G3', 'C4', 'E4', 'G3', 'C4'], t, 0.11, 'triangle', 0.3, 4)
        break
      case 'lose':
        ...
        break
      case 'unlock':
        this.arp(['C5', 'G5', 'C6', 'E6', 'G6', 'C7'], t, 0.07, 'square', 0.12, 5)
        break
    }
  }
}
